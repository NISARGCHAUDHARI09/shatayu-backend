...
import { saveDraftBill } from '/src/utils/draftBillUtils.js';
...
function OPDComponent() {
  ...
  // Fetch OPD patients from authenticated API
  useEffect(() => {
    const fetchOPDPatients = async () => {
      try {
        setLoading(true);
        
        // Check if user has doctor/admin access
        if (!user || (user.role !== 'doctor' && user.role !== 'admin')) {
          console.warn('OPD access requires doctor or admin role');
          setLoading(false);
          return;
        }
        
        const response = await apiClient.getOPDPatients(1, 50);
          ...
          {selectedComplaintsPatient ? (
            <VStack spacing={4} align="stretch">
              ...
              <Card border="1px solid" borderColor="gray.200">
                ...
                <CardBody pt={0}>
                  <VStack spacing={4} align="stretch">
                    {/* Sample previous appointments - in real app, this would come from database */}
                    <Box p={4} border="1px solid" borderColor="gray.100" borderRadius="md" bg="gray.25">
                      <HStack justify="space-between" mb={2}>
                        <Text fontWeight="medium" color="gray.700">Appointment Date: 2024-09-15</Text>
                        <Badge colorScheme="gray" variant="outline">Previous</Badge>
                      </HStack>
                      <VStack spacing={2} align="stretch">
                        <HStack spacing={3} align="start">
                          <Badge colorScheme="blue" variant="outline" fontSize="xs" minW="60px" textAlign="center">1</Badge>
                          <Box flex="1">
                            <Text fontWeight="medium" color="gray.700">Headache and dizziness</Text>
                            <Text fontSize="sm" color="gray.600">Duration: 3 days</Text>
                          </Box>
                        </HStack>
                        <HStack spacing={3} align="start">
                          <Badge colorScheme="blue" variant="outline" fontSize="xs" minW="60px" textAlign="center">2</Badge>
                          <Box flex="1">
                            <Text fontWeight="medium" color="gray.700">Stomach pain after meals</Text>
                            <Text fontSize="sm" color="gray.600">Duration: 1 week</Text>
                          </Box>
                        </HStack>
                      </VStack>
                    </Box>
                    ...
                    <Box p={4} border="1px solid" borderColor="gray.100" borderRadius="md" bg="gray.25">
                      <HStack justify="space-between" mb={2}>
                        <Text fontWeight="medium" color="gray.700">Appointment Date: 2024-08-28</Text>
                        <Badge colorScheme="gray" variant="outline">Previous</Badge>
                      </HStack>
                      <VStack spacing={2} align="stretch">
                        <HStack spacing={3} align="start">
                          <Badge colorScheme="blue" variant="outline" fontSize="xs" minW="60px" textAlign="center">1</Badge>
                          <Box flex="1">
                            <Text fontWeight="medium" color="gray.700">Joint pain in knees</Text>
                            <Text fontSize="sm" color="gray.600">Duration: 2 weeks</Text>
                          </Box>
                        </HStack>
                      </VStack>
                    </Box>

                    {/* Empty state for when no previous history */}
                    <Text color="gray.500" textAlign="center" py={4} fontStyle="italic">
                      * Previous appointment history would be loaded from database
                    </Text>
                  </VStack>
                </CardBody>
              </Card>
                  ...
                  <VStack spacing={2} align="stretch">
                    ...
                    <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
                      <Box>
                        ...
                        <Text fontWeight="bold" color="blue.800">3</Text>
                      </Box>
                      <Box>
                        <Text fontSize="sm" color="blue.600">Most Recent Visit:</Text>
                        <Text fontWeight="bold" color="blue.800">
                          {selectedComplaintsPatient.appointmentDate}
                        </Text>
                      </Box>
                    </SimpleGrid>
                  </VStack>
