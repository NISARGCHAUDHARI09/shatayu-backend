...
const mockSMSMessages = [
  { id: 'SMS001', recipient: 'Rajesh Kumar (+91 9876543210)', message: 'Appointment tomorrow 10:00 AM.', status: 'delivered', timestamp: Date.now(), type: 'reminder', cost: '₹0.50', template: 'Appointment' },
  { id: 'SMS002', recipient: 'Meera Patel (+91 9876543211)', message: 'Lab reports ready.', status: 'sent', timestamp: Date.now(), type: 'lab', cost: '₹0.50', template: 'Lab' }
];
const mockWhatsAppMessages = [
  { id: 'WA001', recipient: 'Anil Singh (+91 9876500001)', message: 'Diet plan shared.', status: 'read', timestamp: Date.now(), type: 'diet', cost: '₹0.20', template: 'Diet', media: false },
  { id: 'WA002', recipient: 'Fatima Ali (+91 9876500002)', message: 'Payment received.', status: 'delivered', timestamp: Date.now(), type: 'payment', cost: '₹0.20', template: 'Payment', media: true }
];
const mockCampaigns = [
  { id: 'CAM001', name: 'Diabetes Camp', type: 'SMS', recipients: 120, delivered: 110, status: 'completed', cost: '₹60', startDate: '2025-09-01', description: 'Free screening week' },
  { id: 'CAM002', name: 'Wellness Followup', type: 'WhatsApp', recipients: 80, delivered: 55, status: 'in_progress', cost: '₹32', startDate: '2025-09-05', description: 'Follow-up reminders' }
];
...
const Messaging = () => {
  ...
  const smsStats = {
    ...
    cost: mockSMSMessages.reduce((s, m) => s + parseFloat(m.cost.replace('₹','')), 0)
  };
  const whatsappStats = {
    total: mockWhatsAppMessages.length,
    delivered: mockWhatsAppMessages.filter(m => m.status === 'delivered' || m.status === 'read').length,
    cost: mockWhatsAppMessages.reduce((s, m) => s + parseFloat(m.cost.replace('₹','')), 0)
  };
  const campaignStats = {
    ...
    <VStack align="stretch" spacing={8} p={6}>
      ...
      <SimpleGrid columns={{ base:1, md:2, lg:4 }} spacing={6}>
        {[{icon:MessageSquare,color:'blue',value:smsStats.total+whatsappStats.total,label:'Total Messages',sub:'+12% MoM'},
          ...
          {icon:TrendingUp,color:'purple',value:campaignStats.active,label:'Active Campaigns',sub:`${campaignStats.completed} completed`},
          {icon:Activity,color:'orange',value:`₹${smsStats.cost.toFixed(0)}`,label:'SMS Cost Today',sub:'-8% MoM'}].map((c,i)=>(
            ...
            <TabPanels>
              ...
              <TabPanel>
                <VStack align="stretch" spacing={4}>
                  ...
                  <TableContainer>
                    <Table size="sm">
                      <Thead><Tr><Th>Name</Th><Th>Status</Th><Th>Delivered</Th><Th>Recipients</Th><Th>Cost</Th><Th>Actions</Th></Tr></Thead>
                      <Tbody>
                        {mockCampaigns.map(c => (
            ...
            {selectedCampaign && (
              <VStack align="stretch" spacing={3} fontSize="sm">
                ...
                <SimpleGrid columns={2} spacing={3}>
                  ...
                  <Box><Text fontWeight="semibold">Cost</Text><Text>{selectedCampaign.cost}</Text></Box>
          ...
          <ModalBody>
            {analyticsCampaign && (
              <VStack align="stretch" spacing={4}>
                ...
                <HStack justify="space-between"><Text>Cost</Text><Text>{analyticsCampaign.cost}</Text></HStack>
          ...
          <ModalBody>
            <VStack align="stretch" spacing={4}>
              ...
              <FormControl>
                ...
                <Input value={composeRecipient} onChange={e=>setComposeRecipient(e.target.value)} placeholder="Enter patient name" />
              </FormControl>
              <FormControl>
                <FormLabel>Message</FormLabel>
                <Textarea rows={4} value={composeMessage} onChange={e=>setComposeMessage(e.target.value)} placeholder="Type message..." />
              </FormControl>
            </VStack>
          </ModalBody>
          <ModalFooter>
            <Button colorScheme="blue" mr={3} onClick={()=>{composeDisc.onClose();}}>Send Message</Button>
            <Button variant="outline" onClick={composeDisc.onClose}>Cancel</Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
