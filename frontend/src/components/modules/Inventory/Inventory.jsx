...
const Inventory = ({ title = "Inventory Management", showAddButton = true }) => {
  ...
  const handleImportFile = (e) => {
    ...
    reader.onload = async (evt) => {
      ...
      if (file.name.endsWith('.csv')) {
        ...
        items = window.XLSX.utils.sheet_to_json(worksheet, { defval: '' });
      } else {
        alert('XLSX library not loaded for Excel import.');
        return;
      }

      // Send items to backend API
      try {
        const response = await axios.post('/api/inventory/import', { items });
        if (response.data.success) {
          alert(`Successfully imported ${response.data.data.imported} items!${response.data.data.errors > 0 ? ` (${response.data.data.errors} errors)` : ''}`);
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Name', 'Category', 'Stock', 'Unit Price', 'Supplier'];
  ...
  const handleExportExcel = () => {
    // Example Excel export logic using XLSX
    if (!window.XLSX) {
    ...
    window.XLSX.writeFile(wb, 'inventory_export.xlsx');
  };
  ...
  // Add Supplier Modal state and form
  ...
  // Handler to add supplier
  const handleAddSupplier = async () => {
    try {
      await axios.post('/api/suppliers', newSupplier);
      addSupplierModal.onClose();
      setNewSupplier({ name: '', category: 'company', type: '', contact: '', email: '', address: '' });
  ...
  // const [isLoading, setIsLoading] = useState(true); // Removed duplicate declaration
  ...
  const fetchInventoryStats = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        // You can use these stats in your component state if needed
  ...
  // Fetch suppliers (if you have suppliers API)
  useEffect(() => {
            ...
            {showAddButton && (
              <HStack spacing={3}>
                ...
                <Menu>
                  ...
                  <MenuList zIndex={9999} bg="white" color="gray.800" border="1px solid #e2e8f0">
                    <MenuItem onClick={handleExportPDF} bg="white" color="gray.800" _hover={{ bg: 'gray.100' }}>Export as PDF</MenuItem>
                    <MenuItem onClick={handleExportCSV} bg="white" color="gray.800" _hover={{ bg: 'gray.100' }}>Export as CSV</MenuItem>
                    <MenuItem onClick={handleExportExcel} bg="white" color="gray.800" _hover={{ bg: 'gray.100' }}>Export as Excel</MenuItem>
                  </MenuList>
                </Menu>
                <Button 
                  bgGradient="linear(to-r, blue.500, blue.700)"
            ...
            <TabPanels>
                        ...
                        >
                              ...
                              <HStack spacing={3}>
                                ...
                                <VStack align="start" spacing={0}>
                                  ...
                                  <HStack fontSize="xs" color={textSecondary} spacing={3}>
                                    ...
                                    <Text>•</Text>
                                    <Text>Batch: {item.batch_number || 'N/A'}</Text>
                                  </HStack>
                            ...
                            <VStack align="start" spacing={3}>
                              ...
                              <Box w="120px">
                                ...
                                <Progress 
                                  ...
                                  colorScheme={getStockColor(item.current_stock, item.min_stock, item.max_stock)}
                                  size="lg"
                                  borderRadius="md"
                                  bg="gray.100"
                                />
                          ...
                          <Td py={4}>
                            <VStack align="start" spacing={1}>
                              ...
                              <Text fontSize="sm" color={textSecondary}>
                                {formatCurrency(item.unit_price)} per {item.unit.toLowerCase().slice(0, -1)}
                              </Text>
                              <HStack spacing={1}>
                                <Text fontSize="xs" color={textSecondary}>Qty:</Text>
                                <Text fontSize="xs" fontWeight="medium">{item.current_stock} {item.unit.toLowerCase()}</Text>
                              </HStack>
                            </VStack>
                          </Td>
                          <Td py={4}>
                          ...
                          <Td py={4}>
                            {item.manufacturing_date ? (
                              <VStack align="start" spacing={2}>
                                ...
                                <Badge colorScheme="cyan" variant="outline" size="sm">
                                  Manufacturing Date
                                </Badge>
                              </VStack>
                            ) : (
                              <VStack align="start" spacing={1}>
                                <Text fontSize="sm" color={textSecondary}>No MFD</Text>
              ...
              <TabPanel p={0}>
                <VStack align="stretch" spacing={6}>
                    ...
                    <CardHeader bg={useColorModeValue('gray.50', 'gray.800')} py={4}>
                      <HStack justify="space-between">
                        ...
                        <HStack spacing={2}>
                          ...
                          <Button colorScheme="purple" leftIcon={<Plus />} size="sm" onClick={addSupplierModal.onOpen}>
                            Add Supplier
                          </Button>
                        </HStack>
                      </HStack>
                  ...
                  {/* Add Supplier Modal */}
                  <Modal isOpen={addSupplierModal.isOpen} onClose={addSupplierModal.onClose} size="lg">
                    <ModalOverlay />
                    <ModalContent>
                      ...
                      <ModalBody>
                        <VStack spacing={4} align="stretch">
                          ...
                          <FormControl>
                            ...
                            <Input value={newSupplier.type} onChange={e => setNewSupplier({ ...newSupplier, type: e.target.value })} />
                          </FormControl>
                          <FormControl>
                      ...
                      <ModalFooter>
                        <Button colorScheme="purple" mr={3} onClick={handleAddSupplier}>Add Supplier</Button>
                        <Button variant="outline" onClick={addSupplierModal.onClose}>Cancel</Button>
                      </ModalFooter>
                    </ModalContent>
                  </Modal>
                    <CardBody p={0}>
        ...
        <ModalContent>
          ...
          <ModalBody>
            <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
              ...
              <FormControl>
                <FormLabel>Manufacturing Date (MFD)</FormLabel>
                <Input
                  type="date"
              ...
              onClick={async () => {
                try {
                  ...
                  const itemData = {
                    ...
                    unit: 'pieces', // Default unit, you can add unit selection to form
                    current_stock: newItem.currentStock,
                    min_stock: newItem.minStock,
                    max_stock: newItem.maxStock,
                    unit_price: newItem.unitPrice,
                    supplier: newItem.supplier,
                    manufacturing_date: newItem.manufacturingDate,
                    expiry_date: newItem.expiryDate,
                    location: newItem.location,
                    reorder_point: newItem.minStock // Use min stock as reorder point
                  };
              ...
              <VStack align="stretch" spacing={4}>
                ...
                <SimpleGrid columns={2} spacing={4}>
                  ...
                  <Box>
                    ...
                    <Text fontWeight="medium">{selectedItem.current_stock} {selectedItem.unit}</Text>
                  </Box>
                  <Box>
                    <Text fontSize="sm" color={textSecondary} mb={1}>Unit Price</Text>
                    <Text fontWeight="medium">{formatCurrency(selectedItem.unit_price)}</Text>
                  </Box>
                  <Box>
                    <Text fontSize="sm" color={textSecondary} mb={1}>Total Value</Text>
                    <Text fontWeight="bold" color="green.600">{formatCurrency(selectedItem.total_value)}</Text>
                  </Box>
                  <Box>
                ...
                <Box>
                  ...
                  <Progress
                    value={getStockPercentage(selectedItem.current_stock, selectedItem.max_stock)}
                    colorScheme={getStockColor(selectedItem.current_stock, selectedItem.min_stock, selectedItem.max_stock)}
                    size="lg"
                    borderRadius="md"
                  />
                  <HStack justify="space-between" mt={2}>
                    <Text fontSize="sm">Min: {selectedItem.min_stock}</Text>
                    <Text fontSize="sm">Current: {selectedItem.current_stock}</Text>
                    <Text fontSize="sm">Max: {selectedItem.max_stock}</Text>
                  </HStack>
                </Box>
              </VStack>
            )}
